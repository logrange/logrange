...
func (rs *rpcScnanner) Run(ctx context.Context) error {
	...
	for ctx.Err() == nil {
		...
		if err != nil {
			rs.disconnect()
			sleepDur := time.Second
			rs.logger.Error("Could not make server call query. err=", err, " will try again in ", sleepDur)
			rs.sleep(ctx, sleepDur)
			continue
		}
		...
		if qr.Err != nil {
			sleepDur := time.Second
			rs.logger.Warn("got query execution error=", qr.Err, ", will try the request once again in ", sleepDur)
			rs.sleep(ctx, sleepDur)
			continue
		}
