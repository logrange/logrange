...
type (
	...
	Service interface {
		...
		// VF_DO_NOT_RELEASE will not release the journal automatically, but it is the client responsibility to release
		// the journal later
		...
		// If it returns true, the journal will not be returned in Visit and GetOrCreateJournal will be blocked unitl
		// it is released. The journal must be un-locked the exclusivelies, it was not deleted.
		LockExclusively(jn string) bool

		// UnlockExclusively removes the exclusive lock from the journal. If the lock is not acquired, will panic
		UnlockExclusively(jn string)

		// Release allows to release the journal name which could be acquired by GetOrCreateJournal
		Release(jn string)

		// Delete allows to delete a journal. It must be exclusively locked before the call. If the journal
		// was deleted, the consequireve Release() call will not have any effect. If the Delete returns any
		// error, the journal must be unlocked and released if it was acquired before
	...
	// over the visit set until it is over or the function returns false. While the function is called the journal
