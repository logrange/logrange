...
func NewK8sJsonParser(fileName string, maxRecSize int, ctx context.Context) (*K8sJsonLogParser, error) {
	...
	jp.fn = fileName
	jp.f = f
	jp.lr = newLineReader(f, maxRecSize, ctx)
	return jp, nil
}

func (jp *K8sJsonLogParser) NextRecord() (*model.Record, error) {
	line, err := jp.lr.readLine()
	if err != nil {
		return nil, err
	}
...
func (jp *K8sJsonLogParser) GetStats() *parser.Stats {
