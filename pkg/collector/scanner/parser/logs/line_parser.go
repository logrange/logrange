...
type (
	...
	lineParser struct {
		...
		stats    *parser.Stats // parser statistics
...
func NewLineParser(fileName string, dateParser date.Parser, maxRecSize int, ctx context.Context) (*lineParser, error) {
	...
	lp := new(lineParser)
	lp.f = f
	lp.lr = newLineReader(f, maxRecSize, ctx)
	lp.stats = parser.NewTxtStats()
	lp.state = parsing
...
func (lp *lineParser) NextRecord() (*model.Record, error) {
	line, err := lp.lr.readLine()
	if err != nil {
		return nil, err
	}
...
func (lp *lineParser) GetStats() *parser.Stats {
